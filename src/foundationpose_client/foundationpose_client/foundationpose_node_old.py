...
import cv2
from interfaces.srv import GetVisionData, AssemblyCommand, ExecuteAssembly
...
class FoundationPoseClientNode(Node):
    def __init__(self):
        ...
        while not self.main_task_client.wait_for_service(timeout_sec=1.0):
    ...
    def _print_pose_result(self, result: dict):
        ...
        # req = ExecuteAssembly.Request()
        # if self.current_face == None:
        #     self.get_logger().info("face_in is None")
        #     return
        # req.face_id = self.current_face
        # req.est_pos = self.set_req_posx(posx)
        # self.get_logger().info(f"create req : {req}")
        # future = self.main_task_client.call_async(req)
        # future.add_done_callback(self.main_task_callback)        
        # self.get_logger().info("===========================================")
        # self.get_logger().info("call robot service")
        # self.get_logger().info("===========================================")
    
    def set_req_posx(self, pos):
        pos[2] = 0
        pos[4] = 0
        posx_float = [float(round(x, 2)) for x in pos]
        print(posx_float)
        return posx_float
    
    def main_task_callback(self, future):
        try:
    ...
    def voice_request_callback(self, request, response):
        ...
        self.current_face = request.face_id
